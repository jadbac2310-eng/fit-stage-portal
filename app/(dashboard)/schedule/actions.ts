...
function toTimes(
  ...
  if (!startDate) throw new ActionError("開始日を入力してください");
...
export async function createPersonalEventsAction(formData: FormData): Promise<ActionResult> {
  return runAction(async () => {
    const member = await getCurrentMember();
    if (!member) throw new ActionError("ログインが必要です");
    ...
    if (slots.length === 0) throw new ActionError("日時を入力してください");
    if (slots.length > 200) throw new ActionError("作成できる日時は最大200件です。繰り返しの終了日を見直してください。");
...
export async function updatePersonalEventAction(id: string, formData: FormData): Promise<ActionResult> {
  return runAction(async () => {
    const [member, event] = await Promise.all([getCurrentMember(), getPersonalEvent(id)]);
    if (!member) throw new ActionError("ログインが必要です");
    if (!event)  throw new ActionError("予定が見つかりません");
    if (event.memberId !== member.id && !member.isAdmin) {
      throw new ActionError("この予定を編集できるのは作成者または管理者のみです");
    }
...
export async function deletePersonalEventAction(id: string): Promise<ActionResult> {
  return runAction(async () => {
    const [member, event] = await Promise.all([getCurrentMember(), getPersonalEvent(id)]);
    if (!member) throw new ActionError("ログインが必要です");
    if (!event)  throw new ActionError("予定が見つかりません");
    if (event.memberId !== member.id && !member.isAdmin) {
      throw new ActionError("この予定を削除できるのは作成者または管理者のみです");
    }
...
function hourlyTaskTimes(formData: FormData): { scheduledAt: string; endAt: string } {
  ...
  const endDate   = (formData.get("endDate")   as string)?.trim() || startDate;
  const endTime   = (formData.get("endTime")   as string)?.trim();
  if (!startDate || !startTime || !endTime) throw new ActionError("開始・終了の日時を入力してください");
  return { scheduledAt: toIso(startDate, startTime), endAt: toIso(endDate, endTime) };
}

export async function createHourlyTaskAction(formData: FormData): Promise<ActionResult> {
  return runAction(async () => {
    await requireAdmin();
    ...
    if (!memberId || !title || !Number.isFinite(hourlyRate)) {
      throw new ActionError("必須項目が未入力です");
    }
...
export async function updateHourlyTaskAction(id: string, formData: FormData): Promise<ActionResult> {
  return runAction(async () => {
    await requireAdmin();
