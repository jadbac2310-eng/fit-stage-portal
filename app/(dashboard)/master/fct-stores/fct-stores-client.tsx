...
import { Plus, Pencil, Trash2, X, MapPin, Landmark } from "lucide-react";
import { Spinner } from "@/components/ui/spinner";
import { useSubmitLock } from "@/lib/use-submit-lock";
import type { FctStore } from "@/lib/fct-stores";
import { createFctStoreAction, updateFctStoreAction, deleteFctStoreAction } from "./actions";

function yen(n: number) {
  return `¥${n.toLocaleString("ja-JP")}`;
}

function FctStoreForm({
  defaultValues, onClose, action, submitLabel,
}: {
  defaultValues?: Partial<FctStore>;
  onClose: () => void;
  ...
  return (
    <form action={handleSubmit} className="space-y-4">
      <div>
        <label className={labelClass}>FCT店舗名 <span className="text-red-500">*</span></label>
        <input name="name" required autoFocus defaultValue={defaultValues?.name} placeholder="FCT○○店" className={inputClass} />
      </div>
      <div>
...
function FctStoreRow({ store, isAdmin }: { store: FctStore; isAdmin: boolean }) {
  const router = useRouter();
  const [mode, setMode] = useState<"view" | "edit">("view");
  const { locked: deleting, run: runDelete } = useSubmitLock();
  const boundUpdate = updateFctStoreAction.bind(null, store.id);

  function handleDelete() {
    if (deleting) return;
    if (!confirm(`「${store.name}」を削除しますか？`)) return;
    runDelete(async () => {
      await deleteFctStoreAction(store.id);
      router.refresh();
    });
  }
    ...
    return (
      <div className="bg-white rounded-2xl border-2 border-blue-400 p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-bold text-gray-900">FCT店舗を編集</p>
          <button onClick={() => setMode("view")} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
        </div>
        <FctStoreForm defaultValues={store} onClose={() => setMode("view")} action={boundUpdate} submitLabel="保存する" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-4 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs font-semibold text-rose-600 mt-1">{yen(store.fee)} / 回</p>
      </div>
      {isAdmin && (
...
export function FctStoresClient({ stores, isAdmin }: { stores: FctStore[]; isAdmin: boolean }) {
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto pb-10">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-gray-900">FCT店舗マスタ</h1>
          </div>
          <p className="text-sm text-gray-500 mt-0.5">名前・住所・料金を管理（{stores.length}件）</p>
        </div>
        {isAdmin && !showAdd && (
      ...
      {showAdd && (
        <div className="bg-white rounded-2xl border-2 border-blue-400 p-5 mb-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-bold text-gray-900">新しいFCT店舗</p>
            <button onClick={() => setShowAdd(false)} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
          </div>
          <FctStoreForm onClose={() => setShowAdd(false)} action={createFctStoreAction} submitLabel="追加する" />
        </div>
      )}
      ...
      {stores.length === 0 && !showAdd ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm font-semibold text-gray-600">FCT店舗が登録されていません</p>
        </div>
      ) : (
        <div className="space-y-2">
          {stores.map((g) => <FctStoreRow key={g.id} store={g} isAdmin={isAdmin} />)}
        </div>
      )}
    </div>
  );
