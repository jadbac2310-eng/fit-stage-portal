  ...
  return (
    <form action={handleSubmit} className="space-y-4">
      <div>
        ...
        <input name="name" required autoFocus defaultValue={defaultValues?.name} placeholder="○○店" className={inputClass} />
      </div>
      <div>
    ...
    return (
      <div className="bg-white rounded-2xl border-2 border-blue-400 p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-bold text-gray-900">店舗を編集</p>
          <button onClick={() => setMode("view")} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
        </div>
        <StoreForm defaultValues={store} onClose={() => setMode("view")} action={boundUpdate} submitLabel="保存する" />
      </div>
    );
  }
...
export function StoresClient({ stores, isAdmin }: { stores: Store[]; isAdmin: boolean }) {
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto pb-10">
      ...
      {showAdd && (
        <div className="bg-white rounded-2xl border-2 border-blue-400 p-5 mb-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-bold text-gray-900">新しい店舗</p>
            <button onClick={() => setShowAdd(false)} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
          </div>
          <StoreForm onClose={() => setShowAdd(false)} action={createStoreAction} submitLabel="追加する" />
        </div>
      )}
      ...
      {stores.length === 0 && !showAdd ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm font-semibold text-gray-600">店舗が登録されていません</p>
        </div>
      ) : (
        <div className="space-y-2">
          {stores.map((s) => <StoreRow key={s.id} store={s} isAdmin={isAdmin} />)}
        </div>
      )}
    </div>
  );
